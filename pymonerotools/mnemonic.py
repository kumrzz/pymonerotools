...
import ecdsa
import pbkdf2
import Crypto.Random.random as randcrypto
import zlib
...
class Mnemonic(object):
    ...
    def make_seed(self, seed_type='standard', num_bits=128, custom_entropy=1):
        ...
        bpw = math.log(len(self.wordlist), 2)#len=2048 so bpw=11
        num_bits = int(math.ceil(num_bits/bpw)) * bpw#equals 132
        # handle custom entropy; make sure we add at least 16 bits
        n_custom = int(math.ceil(math.log(custom_entropy, 2)))#zero@cust=1
        n = max(16, num_bits - n_custom)#132forBTC
    ...
    def electrumChecksum(self, seedinit):
    ...
    def makemoneroseed(self):
        ...
        for k in range(24):
            nonce += 1
            my_entropy = ecdsa.util.randrange(pow(2, 256))
            i = custom_entropy * (my_entropy + nonce)
            x = i % n
            words.append(self.wordlist[x])
        seedinit = ' '.join(words)
        seed = seedinit + ' ' + self.electrumChecksum(seedinit)
        #print randcrypto.getrandbits(256)
        return seed